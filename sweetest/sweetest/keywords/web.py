...
def open(step):
    ...
    if cookie:
        #logger.info(json2dict(cookie))
        cookies = json2dict(cookie)
        for k in cookies:
            ck = {"name": k, "value": str(cookies.get(k))}
            logger.info(f'add cookie {ck} ')
            g.driver.add_cookie(ck)
...
def up_file(step):
    ...
    for key in data:
        if key.startswith('text'):
            if isinstance(data[key], tuple):
                #file_path = path1 + '\\pic' + data[key]
                element_location.send_keys(*data[key])
            elif element_location:
                file_path = file_dir + '\\file\\' + data[key]
...
def upload(step):
    ...
    element_location = locating_element(element)
    file_dir = os.getcwd()
    file_name = data.get('text', '') or data.get('file', '')
